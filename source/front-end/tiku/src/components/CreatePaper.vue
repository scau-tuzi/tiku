<template>
  <el-container>
    <el-header height="30px">
      <el-page-header @back="goBack" content="新建试卷"></el-page-header>
    </el-header>
    <el-main>
      <el-row>
        <el-col :span="3">
  ...
  methods: {
    ...
    complete() {
      ...
      if (tableData.length === 0) {
        this.$message({ type: "warning", message: "问题数为0" });
        return; // todo
      }
      ...
      new Sortable(tbody[0], {
        ...
        onEnd: function(evt) {
          ...
          if (tbody[0] === evt.from && tbody[1] === evt.to) {
            ...
            _this.createPaperOrderMock.tableData.splice(
              evt.newIndex,
              0,
              currRow
            );
      ...
      new Sortable(tbody[1], {
        ...
        onEnd: function(evt) {
          if (tbody[1] === evt.from && tbody[1] === evt.to) {
            const currRow = _this.createPaperOrderMock.tableData.splice(
              evt.oldIndex,
              1
            )[0];
            _this.createPaperOrderMock.tableData.splice(
