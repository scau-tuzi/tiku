<template>
  <el-container>
    <el-header height="30px">
      <el-page-header @back="goBack" content="编辑试卷"></el-page-header>
    </el-header>
    <el-main>
      <el-row>
        <el-col :span="3">
      ...
      <el-row>
        <el-col :span="12.5" style="margin-right: 10px">
          <GeneralTable
            usePagination
            :handleChange="leftTablePageChange"
            :listSize="listSize"
            v-bind:table-info="leftTable"
            v-on:handleButton="handleButtonLeft"
          ></GeneralTable>
        </el-col>
        <el-col :span="11">
          <GeneralTable v-bind:table-info="modifyPaperOrderMock" v-on:handleButton="handleButton"></GeneralTable>
        </el-col>
      </el-row>
      <!--      <el-row>-->
...
} from "../data/mock/CreatePaperInfoMock";
    ...
    pd.filter(v => {
      ...
      if (v.extData !== null) {
        for (let j = 0; j < Object.keys(v.extData).length; j++) {
          ext.push(
            Object.keys(v.extData)[j] + ":" + Object.values(v.extData)[j]
          );
        }
      }
      let ress = {
...
export default {
  ...
  methods: {
    ...
    editPaper() {
      ...
      for (
        let i = 0;
        i < this.$store.state.paperEditData.problems.length;
        i++
      ) {
        let ts = [];
        let ext = [];
    ...
    complete() {
      ...
      if (tableData.length === 0) {
        this.$message({ type: "warning", message: "问题数为o" });
        return; // todo
      }
      ...
      new Sortable(tbody[0], {
        ...
        onEnd: function(evt) {
          if (tbody[0] === evt.from && tbody[0] === evt.to) {
            const currRow = _this.leftTable.tableData.splice(
              evt.oldIndex,
              1
            )[0];
          ...
          if (tbody[0] === evt.from && tbody[1] === evt.to) {
            ...
            _this.modifyPaperOrderMock.tableData.splice(
              evt.newIndex,
              0,
              currRow
            );
      ...
      new Sortable(tbody[1], {
        ...
        onEnd: function(evt) {
          if (tbody[1] === evt.from && tbody[1] === evt.to) {
            const currRow = _this.modifyPaperOrderMock.tableData.splice(
              evt.oldIndex,
              1
            )[0];
            _this.modifyPaperOrderMock.tableData.splice(
